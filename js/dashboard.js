import { captureToClipboard, fmt, fmtShort, toNum, parseDate, formatDate, tagClass, showGlobalLoading, hideGlobalLoading } from "./ui.js";
import { storage } from "./firebase-config.js";
import { ref, getDownloadURL } from "https://www.gstatic.com/firebasejs/10.12.4/firebase-storage.js";
...
export function initDashboard() {
  ...
  showGlobalLoading("Verificando planilha...");

  unsubPlanilha = listenPlanilhaMetadata(async (data) => {
    hideGlobalLoading();

    const adminInfo = document.getElementById('adminCurrentInfo');
    if (adminInfo && data) {
      const date = new Date(data.updatedAt).toLocaleString('pt-BR');
      adminInfo.innerHTML = `<strong>${data.filename}</strong><br><span style="color:var(--muted);font-size:12px">Atualizado em ${date} por ${data.updatedBy}</span>`;
    } else if (adminInfo) {
    ...
    try {
      showGlobalLoading("Processando planilha...");
      ...
      document.getElementById('savedBadge').innerHTML = `
        ...
        Sincronizado em ${new Date().toLocaleTimeString('pt-BR')}`;
      hideGlobalLoading();
    } catch (err) {
      hideGlobalLoading();
      console.error(err);
...
function renderDashboard() {
  ...
  document.getElementById('main-content').innerHTML = `
    ...
    <div class="months-grid">${months.map(renderMonthCard).join('')}</div>
